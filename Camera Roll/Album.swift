...
class Album {
  ...
  class func allAlbums() -> [Album] {
    ...
    let today = NSDate()
    let yesterday = calendar.dateByAddingUnit(NSCalendarUnit.Day, value: -1, toDate: today, options: [])!
    let theDayBeforeYesterday = calendar.dateByAddingUnit(NSCalendarUnit.Day, value: -2, toDate: today, options: [])!
    let theDayBeforeThat = calendar.dateByAddingUnit(NSCalendarUnit.Day, value: -3, toDate: today, options: [])!
